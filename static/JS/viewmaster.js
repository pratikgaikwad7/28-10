...
// Helper function to determine font size based on bar count
function getFontSizeForBarCount(barCount) {
  if (barCount <= 4) return 18;
  // Few bars → big labels
  else if (barCount <= 7) return 15;
  // Medium → medium labels
  else if (barCount <= 12) return 13;
  // More bars → smaller labels
  else return 13; // Very many → smallest labels
}
...
function initializeChartSelection() {
  const availableCharts = [
    ...
    {
      id: "factoryChart-container",
      title: "Factory Training Statistics",
      icon: "fa-chart-bar",
    },
    {
...
async function downloadChartAsEnlarged(chartId) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Get bar count to determine font size and canvas width
      const barCount = enlargedConfig.data.labels.length;
      const fontSize = getFontSizeForBarCount(barCount);

      // Determine canvas dimensions based on bar count
      let canvasWidth = 1200;
      if (barCount > 8) {
        canvasWidth = 2400; // Double width for charts with more than 8 bars
      }
      ...
      downloadCanvas.style.width = `${canvasWidth - 80}px`; // Account for padding
      ...
      // 1. Title
      if (
        enlargedConfig.options.plugins &&
        enlargedConfig.options.plugins.title
      ) {
        enlargedConfig.options.plugins.title.font = {
          ...(enlargedConfig.options.plugins.title.font || {}),
          size: fontSize + 7, // Title is the largest
        };
      }
      ...
      if (
        ...
        enlargedConfig.options.plugins.legend.labels.font = {
          ...(enlargedConfig.options.plugins.legend.labels.font || {}),
          size: fontSize + 2, // Legend a bit larger than data labels
        };
      }

      // 3. Data labels (datalabels plugin)
      if (
        enlargedConfig.options.plugins &&
        enlargedConfig.options.plugins.datalabels
      ) {
        enlargedConfig.options.plugins.datalabels.font = {
          ...(enlargedConfig.options.plugins.datalabels.font || {}),
          size: fontSize,
        };
      }

      // 4. Scales (both ticks and titles)
      if (enlargedConfig.options.scales) {
        Object.keys(enlargedConfig.options.scales).forEach((scaleId) => {
          const scale = enlargedConfig.options.scales[scaleId];
          if (scale.ticks) {
            scale.ticks.font = {
              ...(scale.ticks.font || {}),
              size: fontSize,
      ...
      // 5. Custom valueLabelsPlugin (if present)
      if (enlargedConfig.plugins) {
        enlargedConfig.plugins = enlargedConfig.plugins.map((plugin) => {
          if (plugin && plugin.id === "valueLabelsPlugin") {
            // Replace the afterDatasetsDraw function with one that uses the dynamic fontSize
            return {
              ...plugin,
    ...
    if (originalConfig.options.plugins) {
      ...
      if (originalConfig.options.plugins.datalabels) {
        config.options.plugins.datalabels = {
          ...
          // Font size will be set dynamically in downloadChartAsEnlarged
...
function getChartTitle(chartId) {
  const chartTitles = {
    ...
    "factoryChart-container": "Factory Training Statistics",
